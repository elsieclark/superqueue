...
const executeOnQueue = (queue, flags, defaultFlag, timeout, emitter) => {
    ...
    const flagNextExecutionTimes = {};
    _.forEach(flagNames, (flagName) => {
        flagNextExecutionTimes[flagName] = nextFlagExecutionTime(flags[flagName]);
    });

    // Check if default flag is blocked
    let waitDuration = flagNextExecutionTimes[defaultFlag] - currentTime;
    if (waitDuration > 0) {
        clearTimeout(timeout.timeout);
        setTimeout(executeOnQueue, waitDuration + 1, queue, flags, defaultFlag, timeout, emitter);
        return;
    }
    ...
    clearTimeout(timeout.timeout);
    const nextStartTime = Math.max(flagNextExecutionTimes[defaultFlag], _.min(flagNextExecutionTimes), currentTime);
    timeout.timeout = setTimeout(executeOnQueue, nextStartTime + 1 - currentTime, queue, flags, defaultFlag, timeout, emitter);
};
        ...
        return new Promise((resolve, reject) => {
            ...
            if (_.isFunction(args[0])) {
                ...
                if (_.has(funcOptions, 'name')) {
                    name = validKey(funcOptions.name, 'name');
                }
                if (_.has(funcOptions, 'flags')) {
                    ...
                    funcOptions.flags.forEach((flagName) => {
                        if (!_.isString(flagOptions.name) && !_.isSymbol(flagOptions.name)) {
                            throw Err(`Flag '${flagName}' provided to '.push()' is not a valid flag name.`);
                        }
                        if (!flags[flagName]) {
                            throw Err(`Flag '${flagName}' provided to '.push()' does not exist.`);
                        }
                    });
            ...
            _.forEach(attachedFlags, (flagName) => {
            ...
            queue.splice(_.sortedLastIndexBy(queue, queueObject, 'priority'), 0, queueObject);
            executeOnQueue(queue, flags, defaultFlag, timeout, this);
        });
    };
