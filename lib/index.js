...
const SuperQueue = function (options) {
    ...
    this.addFlag = (flagName, flagOptions) => {
        if (!Valid.stringOrSymbol(flagName) || !flagName) {
            throw Err(`Error creating flag: '${flagName}' is not a valid name.`);
        }
        Flag.init(flags, flagName, flagOptions);
    };
